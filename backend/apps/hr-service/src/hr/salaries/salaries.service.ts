...
export class SalariesService {
  ...
  async getStatistics(year: number, month?: number) {
    ...
    // TỐI ƯU (DBA): Chống Memory Leak. Đẩy toàn bộ tính toán SUM, COUNT, GROUP BY xuống PostgreSQL Engine thay vì dùng JS reduce()
    const [agg, groupStatus, groupMonth, insuranceAgg] = await Promise.all([
      ...
      this.prisma.salaryDetail.aggregate({
        where: {
          salary: where,
          type: DetailType.INSURANCE,
        },
    ...
    const totalNetSalary = agg._sum.netSalary ?? 0;
    const totalBonus = agg._sum.totalBonus ?? 0;
    const totalDeduction = agg._sum.totalDeduction ?? 0;
    const totalInsurance = insuranceAgg._sum.amount ?? 0;
    const totalEmployees = agg._count.id;
    ...
    const monthlyBreakdown = groupMonth.map((row) => ({
      month: row.month,
      total: row._sum.netSalary ?? 0,
      count: row._count.id,
    }));
    ...
    return {
      ...
      totalInsurance,
      avgNetSalary:
        totalEmployees > 0 ? Math.round(totalNetSalary / totalEmployees) : 0,
      byStatus,
      monthlyBreakdown,
    };
  }
