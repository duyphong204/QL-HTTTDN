...
export class OrdersService {
  ...
  async createOrder(userId: string, dto: CreateOrderDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // 5. Emit Event tới RabbitMQ (Gửi cho Warehouse và Report)
      this.rmqClient.emit('order.created', order);
  ...
  async handleStockFailed(orderId: string, reason: string) {
    ...
    if (order.status !== 'CANCELLED') {
      ...
      // Nếu đã thanh toán qua Momo, gọi API Refund tại đây
    }
  }
  ...
  private async calculateSalesStatistics(startDate: Date, endDate: Date) {
    ...
    // 1. TỐI ƯU (DBA): Sử dụng Aggregate và QueryRaw để DB xử lý toán học (SUM), tránh tràn RAM (Memory Leak) khi có hàng triệu OrderDetail
    const [orderAgg, detailAgg] = await Promise.all([
      this.prisma.order.aggregate({
        where: orderWhere,
        _count: { id: true },
        _sum: { totalAmount: true },
      }),
      this.prisma.$queryRaw<{ totalItems: number; totalCost: number }[]>`
        SELECT 
          COALESCE(SUM(od.quantity), 0)::int as "totalItems",
          COALESCE(SUM(od.quantity * od."costPrice"), 0)::float as "totalCost"
        FROM "OrderDetail" od
