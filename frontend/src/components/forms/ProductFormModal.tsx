...
export function ProductFormModal({
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]: ["price", "costPrice", "stockQuantity"].includes(name)
        ? Number(value)
        : value,
    }));
    if (errors[name]) {
  ...
  const validateForm = () => {
    ...
    if (!form.name) newErrors.name = "Tên sản phẩm là bắt buộc";
    if (form.price <= 0) newErrors.price = "Giá bán phải lớn hơn 0";
    if (form.costPrice <= 0) newErrors.costPrice = "Giá nhập phải lớn hơn 0";
        ...
        <div>
          ...
          >
            {imagePreview ? (
              <img
                src={imagePreview}
                className="h-28 object-contain mx-auto rounded-lg"
              />
            ) : (
