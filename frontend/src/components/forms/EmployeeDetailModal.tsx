    ...
    >
      <div className="p-6 md:p-8">
        {isLoading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start pb-6 border-b border-gray-100">
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-gray-600 font-medium">{employee.position}</span>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50/40 rounded-xl p-4 border border-blue-100 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-blue-800 text-sm font-medium">Lương cơ bản (Net):</span>
                    <span className="text-lg font-bold text-blue-700">
                      {formatCurrency(employee.baseSalary)}
                    </span>
                  </div>
                  <p className="text-[10px] text-blue-400 italic leading-relaxed">
                    * Mức lương này là căn cứ để tính toán các khoản phụ cấp và bảo hiểm.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <Clock size={20} className="text-orange-500" /> Lịch sử thay đổi nội bộ
              </h3>
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden bg-white shadow-sm">
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-500 border-b border-gray-100">
                    <tr>
                      <th className="px-6 py-3 font-semibold uppercase text-[11px]">Giai đoạn</th>
                      <th className="px-6 py-3 font-semibold uppercase text-[11px]">Chức vụ & Phòng ban</th>
                      <th className="px-6 py-3 font-semibold text-right uppercase text-[11px]">Mức lương</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {employee.jobHistories && employee.jobHistories.length > 0 ? (
                      employee.jobHistories.map((history) => (
                        <tr key={history.id} className="hover:bg-gray-50/80 transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <p className="text-gray-800 font-semibold">{history.position}</p>
                            <p className="text-gray-500 text-xs">{history.department}</p>
                          </td>
                      ...
                      <tr>
                        <td colSpan={3} className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center gap-3 opacity-20">
                            ...
                            <p className="text-sm font-medium">Chưa ghi nhận lịch sử điều chuyển</p>
                          </div>
                        </td>
                      </tr>
                    )}
          ...
          <div className="text-center py-12 text-gray-400">Không có dữ liệu hiển thị.</div>
        )}
      </div>
    </AppModal>
  )
