...
export default function EmployeeManagement() {
  // Store actions & state
  const {
    employees, meta, filters, loadingEmployees, 
    selectedEmployee, loadingEmployeeDetail,
    fetchEmployees, fetchEmployeeById, setFilters,
    createEmployee, updateEmployee, deleteEmployee, clearSelectedEmployee
  } = useHrEmployeeStore()

  // UI Hooks
  const { modalOpen, editingEntity, openCreateModal, openEditModal, closeModal } = useEntityModal<Employee>()
  ...
  const getPositionBadge = (position?: string) => {
    const config = ROLE_BADGE[position as keyof typeof ROLE_BADGE] || { label: position || '—', color: 'bg-indigo-100 text-indigo-700' }
    return config
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Quản lý hồ sơ và thông tin nhân sự</p>
          </div>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-50">
                {loadingEmployees ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">Không tìm thấy nhân viên.</td></tr>
                ) : (
                  employees.map((emp) => {
                    const badge = getPositionBadge(emp.position)
                    return (
                      <tr key={emp.id} className="hover:bg-gray-50/70 transition-colors group">
                        <td className="px-6 py-4 font-mono text-gray-900">{emp.code}</td>
                        <td className="px-6 py-4 text-gray-600">{emp.user?.profile?.fullName || "—"}</td>
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => fetchEmployeeById(emp.id)} className="p-1.5 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors"><Eye size={18} /></button>
                            <button onClick={() => openEditModal(emp)} className="p-1.5 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Pencil size={18} /></button>
                            <button onClick={() => handleDelete(emp.id, emp.user?.profile?.fullName)} className="p-1.5 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors"><Trash2 size={18} /></button>
                          </div>
                        </td>
                      </tr>
                    )
