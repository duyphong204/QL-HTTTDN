...
export default function UserManagement() {
  ...
  // Hàm xử lý submit truyền xuống Modal
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-50">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-gray-400">Không tìm thấy user nào phù hợp.</td>
                  </tr>
                ) : (
                  users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50/70 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${ROLE_BADGE[user.role].color}`}>
