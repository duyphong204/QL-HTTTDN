...
export const useImportSlipPage = () => {
  ...
  const setSelectedStockIn = useStockInStore(
    (state) => state.setSelectedStockIn,
  );
  const setProducts = useWarehouseReferenceStore((state) => state.setProducts);
  const setSuppliers = useWarehouseReferenceStore(
    (state) => state.setSuppliers,
  );
  ...
  const clearSelectedStockIn = useStockInStore(
    (state) => state.clearSelectedStockIn,
  );
