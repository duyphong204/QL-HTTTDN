...
export const useAdminReportPage = () => {
  const { report, isLoading, setReport, setLoading, setError } =
    useAdminStore();
  ...
  const quickStats = useMemo(
    () => [
      {
        title: "Tổng đơn hàng",
        value: formatNumberVi(report?.sales?.totalOrders ?? 0),
      },
      {
        title: "Doanh thu",
        value: formatCurrencyVnd(report?.sales?.totalRevenue ?? 0),
      },
      {
        title: "Tổng giá trị nhập",
        value: formatCurrencyVnd(report?.warehouse?.totalImportValue ?? 0),
      },
      {
        title: "Tổng quỹ lương",
        value: formatCurrencyVnd(
          (report?.hr?.totalSalaryPaid ?? 0) + (report?.hr?.totalBonus ?? 0),
        ),
      },
    ],
    [report],
  ...
  const detailStats = useMemo(
    () => ({
      sales: [
        {
          label: "Sản phẩm đã xuất",
          value: formatNumberVi(report?.sales?.totalItemsSold ?? 0),
        },
        {
          label: "Lợi nhuận",
          value: formatCurrencyVnd(report?.sales?.totalProfit ?? 0),
          highlight: true,
        },
      ],
      hr: [
        {
          ...
          value: formatNumberVi(report?.hr?.totalEmployees ?? 0),
        },
        {
          label: "Nhân sự đã nghỉ",
          value: formatNumberVi(report?.hr?.totalResigned ?? 0),
        },
        {
          label: "Bảng lương đã tính",
          value: formatNumberVi(report?.hr?.headcount ?? 0),
        },
      ],
      warehouse: [
        {
          label: "Số phiếu nhập",
          value: formatNumberVi(report?.warehouse?.totalStockIns ?? 0),
        },
        {
          label: "Tổng tồn kho",
          value: formatNumberVi(report?.warehouse?.totalStockQuantity ?? 0),
        },
        {
          label: "Loại sản phẩm",
          value: formatNumberVi(report?.warehouse?.totalProductTypes ?? 0),
        },
      ],
    }),
    [report],
